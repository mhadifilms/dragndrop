...
@main
struct dragndropCLI: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "dragndrop",
        abstract: "CLI tool for dragndrop - control uploads from the command line",
        version: "1.0.0",
        subcommands: [
            Status.self,
            Upload.self,
...
struct GlobalOptions: ParsableArguments {
    @Option(name: .shortAndLong, help: "Port to connect to the dragndrop app")
...
enum CLIError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .notConnected:
            return "Not connected to dragndrop"
        case .connectionCancelled:
            return "Connection was cancelled"
        case .noResponse:
            return "No response received"
        case .appNotRunning:
            return "dragndrop app is not running. Please start the app first."
        }
    }
}
...
struct Server: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        abstract: "Run dragndrop in headless server mode (without GUI)"
    )
    ...
    func run() async throws {
        print("Starting dragndrop server on port \(port)...")
