...
struct AuthenticationStep: View {
        ...
        VStack(spacing: 32) {
            ...
            VStack(spacing: 12) {
                ...
                Text(appState.isAuthenticated
                    ...
                    : "Sign in with AWS SSO to start uploading files to S3.")
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
                    .multilineTextAlignment(.center)
            }
                ...
                VStack(spacing: 16) {
                    ...
                    Button {
                        ...
                        HStack(spacing: 8) {
                            ...
                            Text(isAuthenticating ? "Waiting for browser..." : "Sign in with AWS SSO")
                        }
                        .frame(minWidth: 200)
                    }
                    .buttonStyle(.borderedProminent)
                    ...
                    // Alternative: Profile-based login
                    Divider()
                        .frame(width: 200)

                    Text("Or use an existing AWS profile")
                        .font(.caption)
                        .foregroundStyle(.secondary)

                    HStack(spacing: 12) {
                        TextField("Profile name", text: $appState.settings.awsProfileName)
                        ...
                        .disabled(appState.settings.awsProfileName.isEmpty)
                    }
                }
                .animatedAppearance(delay: 0.3)
            } else {
    ...
    private func startLogin() {
        ...
        Task {
            do {
                let loginInfo = try await appState.startSSOLogin()

                // Open browser
                if let url = URL(string: loginInfo.verificationUriComplete) {
                    NSWorkspace.shared.open(url)
                }

                // Poll for completion
                try await appState.completeSSOLogin(
                    accountId: appState.settings.awsAccountId,
                    roleName: appState.settings.awsRoleName,
                    region: appState.settings.awsSSORegion
                )
            } catch {
    ...
    private func loadProfile() {
        // Profile loading would be implemented in the auth service
        // For now, just indicate that it's not yet implemented
        authError = "Profile-based login will be available in a future update"
    }
}
