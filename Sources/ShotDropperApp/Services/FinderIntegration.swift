...
public class FinderIntegration: ObservableObject {
    ...
    /// Sets up handling for shotdropper:// URL scheme
    ...
    /// Handles shotdropper://upload?file=/path/to/file
    ...
    /// Handles shotdropper://open to bring app to front
    private func handleOpenURL(_ url: URL) {
        NSApp.activate(ignoringOtherApps: true)
    }

    /// Handles shotdropper://workflow?id=xxx to select a workflow
    ...
    /// Service handler for "Upload with ShotDropper" service
            ...
            <array>
                <dict>
                    ...
                    <dict>
                        ...
                        <string>Uploads selected files to S3 using ShotDropper</string>
                        <key>AMIconName</key>
                        ...
                        <key>AMName</key>
                        <string>Upload with ShotDropper</string>
                        <key>AMProvides</key>
                        <dict>
                            <key>Container</key>
                            <string>List</string>
                        ...
                        <dict>
                            ...
                            <string>for f in "$@"
        do
            open "shotdropper://upload?file=$f"
        done</string>
    ...
    public static var appleScriptForUpload: String {
        ...
        on run {input, parameters}
            ...
            if fileList is not "" then
                ...
                open location "shotdropper://upload?" & fileList
            end if
    ...
    public static var shellScriptForUpload: String {
        ...
        open "shotdropper://upload?$PARAMS"
        """
    }
}
...
extension FinderIntegration {
    ...
    public static var urlSchemeDocumentation: String {
        """
        ShotDropper URL Schemes:

        1. Upload files:
           shotdropper://upload?file=/path/to/file1&file=/path/to/file2

        2. Open app:
           shotdropper://open

        3. Select workflow:
           shotdropper://workflow?id=<workflow-uuid>

        4. Quick upload with workflow:
           shotdropper://upload?file=/path/to/file&workflow=<workflow-name>
        """
    }
}
