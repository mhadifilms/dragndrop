...
class AppState: ObservableObject {
    ...
    func startSSOLogin() async throws -> SSOLoginInfo {
        ...
        let loginInfo = try await services.authService.initiateSSOLogin(
            startURL: settings.awsSSOStartURL,
            region: settings.awsSSORegion
        )

        return loginInfo
    }

    func completeSSOLogin(accountId: String, roleName: String, region: String) async throws {
        guard let services = services else {
            throw AppError.notInitialized
        }

        let token = try await services.authService.pollForSSOToken()
        _ = try await services.authService.getCredentialsFromSSO(
            accessToken: token,
            accountId: accountId,
            roleName: roleName,
            region: region
